...
export function calcularMediaRegimental(
  ...
  if (feridoEmCombate && f12 < 10) {
    motivoClassificacao = 'Regra Regimental de Proteção a Feridos em Campanha Aplicada (F12 garantido)';
  }

  if (MP >= 15.0 && todasMaiorIgual15 && fatoresNuclearesValidos) {
    ...
    MP >= 11.25 &&
    fatoresNuclearesValidos &&
    contagemNivel5 <= 2 &&
    (!feridoEmCombate ? true : f12 >= 10)
  ) {
    classificacao = 'FAVORÁVEL';
    motivoClassificacao =
      'Média Ponderada >= 11.25, fatores nucleares (F1, F5, F7, F9 e F11) com nível >= 15 e no máximo 2 fatores em nível 5.';
  } else {
    classificacao = 'DESFAVORÁVEL';
    const motivos: string[] = [];
    if (MP < 11.25) motivos.push(`Média ponderada (${MP.toFixed(2)}) inferior a 11.25`);
    if (!fatoresNuclearesValidos) motivos.push('Nota inferior a 15 num ou mais fatores nucleares (F1, F5, F7, F9, F11)');
    if (contagemNivel5 > 2) motivos.push(`Mais de 2 fatores (${contagemNivel5}) avaliados no nível 5 (Insuficiente)`);
    if (feridoEmCombate && f12 < 10) motivos.push('Condição física inferior ao limiar protegido');
    motivoClassificacao = motivos.join('; ') || 'Requisitos regimentais para parecer favorável não satisfeitos.';
  }

  const incoerenciasDetectadas: string[] = [];
  if (notasEfetivas['F11'] === 20 && notasEfetivas['F10'] === 5) {
    incoerenciasDetectadas.push('Fator 11 (Decisão) avaliado com Muito Bom (20), mas Fator 10 (Julgamento) avaliado com Insuficiente (5).');
  }
  if (notasEfetivas['F1'] === 5 && MP >= 15) {
    incoerenciasDetectadas.push('Fator 1 (Integridade) avaliado com Insuficiente (5), incompatível com Média Geral elevada.');
  }
